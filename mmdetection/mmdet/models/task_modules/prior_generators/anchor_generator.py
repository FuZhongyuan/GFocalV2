...
class AnchorGenerator:
    ...
    def sparse_priors(self,
        ...
        x = (prior_idxs // num_base_anchors) % width
        y = (prior_idxs // num_base_anchors) // width
        stride_w, stride_h = self.strides[level_idx]

        shift_xx = x.to(dtype).to(device) * stride_w
        shift_yy = y.to(dtype).to(device) * stride_h
        ...
        # add A anchors (1, A, 4) to K shifts (K, 1, 4) to get
        # shifted anchors (K, A, 4), reshape to (K*A, 4)
        all_anchors = base_anchors[base_anchor_id, :] + shifts
        if self.use_box_type:
            all_anchors = HorizontalBoxes(all_anchors)
