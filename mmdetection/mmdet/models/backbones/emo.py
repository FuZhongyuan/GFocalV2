...
class EMO(nn.Module):
	...
	def __init__(self, dim_in=3, num_classes=1000, img_size=224,
				 ...
				 attn_drop=0., drop=0., drop_path=0., v_group=False, attn_pre=False, pre_dim=0, pretrain=None,
				 return_idx=[1, 2, 3]):
		super().__init__()
		...
		if self.pretrain:
			ckpt = torch.load(self.pretrain, map_location='cpu')
			if 'net_E' in ckpt.keys():
				state_dict = ckpt['net_E']
