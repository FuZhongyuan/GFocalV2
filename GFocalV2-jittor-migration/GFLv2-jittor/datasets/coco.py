...
class COCODataSets(jt.dataset.Dataset):
    def __init__(self, img_root, annotation_path,
        ...
        """
        super(COCODataSets, self).__init__(batch_size=batch_size, shuffle=shuffle)
        ...
        self.transform = None
        self.set_transform()
        self.total_len = len(self.box_info_list)
    ...
    def collate_batch(self, batch):
        ...
        for item in batch:
            ...
            img= np.resize(img, (640, 640, 3)) # todo : 这里要pad一下
            batch_img.append(img)
            target = np.concatenate([item.labels[:, None], item.boxes], axis=-1)
            batch_target.append(target)
            batch_length.append(len(target))
            # a=np.stack(batch_img, axis=0)
        batch_img = np.stack(batch_img, axis=0).transpose(0, 3, 1, 2)
