...
def split_optimizer(model: nn.Module, cfg: dict):
    ...
    # Jittor优化器参数组处理
    optimizer.param_groups.append({'params': param_weight_decay, 'weight_decay': cfg['weight_decay']})
    optimizer.param_groups.append({'params': param_bias})
    return optimizer
...
class IterWarmUpCosineDecayMultiStepLRAdjust(object):
    def __init__(self, init_lr=0.01,
        ...
        self.flag = np.array([warm_up_epoch] + self.milestones + [last_epoch]).astype(np.int32)
...
class EpochWarmUpCosineDecayLRAdjust(object):
    def __init__(self, init_lr=0.01,
        ...
        self.alpha = alpha
        self.bias_idx = bias_idx
        self.flag = np.array([warm_up_epoch, epochs]).astype(np.int32)
        self.flag = np.unique(self.flag)
        self.warm_up_iter = self.warm_up_epoch * iter_per_epoch
