...
def non_max_suppression(prediction: jt.Var,
                        conf_thresh=0.05,
                        iou_thresh=0.5,
                        max_det=300,
                        max_box=2048,
    ...
    out = [None] * bs
    for bi in range(bs):
        batch_predicts_list = [jt.zeros(size=(0, 6)).float()] * len(prediction)
        for lj in range(len(prediction)):
            ...
            batch_predicts = jt.cat([valid_box, valid_val[:, None], valid_cls[:, None]], dim=-1)
            batch_predicts_list[lj] = batch_predicts
        x = jt.cat(batch_predicts_list, dim=0)
        if x.shape[0] == 0:
            continue
        c = x[:, 5:6] * max_box
...
class Scale(nn.Module):
    def __init__(self, init_val=1.0):
        ...
        self.scale = nn.Parameter(jt.Var(init_val), requires_grad=True)
...
class SubNetFC(nn.Module):
    ...
    def execute(self,x):
        ...
        if self.add_mean:
            stat=jt.cat([prob_topk,prob_topk.mean(dim=-1,keepdim=True)],dim=-1)
        else:
            stat=prob_topk
...
class GFocalHead(nn.Module):
    ...
    def build_anchors_delta(self, size=32.):
        ...
        :return: [anchor_num, 4]
        """
        scales = jt.Var(self.anchor_scales).float()
        ratio = jt.Var(self.anchor_ratios).float()
    ...
    def execute(self,xs):
        ...
        for j,x in enumerate(xs):
            ...
            if cls_feat.dtype==jt.float16:
                cls_feat=cls_feat.float()
            if reg_score.dtype==jt.float16:
                reg_score=reg_score.float()
            if reg_feat.dtype==jt.float16:
        ...
        if self.training:
            ...
            for cls_out,reg_out,stride,anchor in zip(cls_outputs,reg_outputs,self.strides,self.anchors):
                ...
                box_xyxy = jt.cat([x1y1, x2y2], dim=-1)
                predicts_out = jt.cat([box_xyxy, cls_out], dim=-1)  # shape=[bs,all_anchor,4+80]
...
if __name__ == '__main__':
    input_Var = jt.rand(size=(4, 8525, 68))
