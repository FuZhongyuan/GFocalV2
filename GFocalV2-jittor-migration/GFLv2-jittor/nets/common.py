...
class FPN(nn.Module):
    ...
    def execute(self, xs):
        ...
        p5 = self.p5_out(f5)
        f4 = f4 + nn.UpsamplingBilinear2d(scale_factor=(f4.shape[2] / f5.shape[2], f4.shape[3] / f5.shape[3]))(f5)
        p4 = self.p4_out(f4)
        f3 = f3 + nn.UpsamplingBilinear2d(scale_factor=(f3.shape[2] / f4.shape[2], f3.shape[3] / f4.shape[3]))(f4)
        p3 = self.p3_out(f3)
