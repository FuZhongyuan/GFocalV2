...
| 指标               | Jittor | PyTorch | 差异    |
|--------------------|--------|---------|---------|
| 平均损失           | 1.8579 | 2.0787  | -0.2208 |
| 最终损失           | 1.5095 | 1.9564  | -0.4469 |
| 最佳 mAP           | 0.0510 | 0.0040  | 0.0470  |
| 最佳 mAP@0.5       | 0.0890 | 00.060  | 0.0830  |
| 小物体 mAP         | 0.0440 | 0.0000  | 0.0440  |
| 中物体 mAP         | 0.0750 | 0.0020  | 0.0730  |
| 大物体 mAP         | 0.1160 | 0.0150  | 0.1010  |
...
5. **内存占用**：Jittor在内存使用效率方面表现优异，在训练和测试过程中平均节省约6.4%的内存。特别是在训练过程中，Jittor的内存使用更加稳定，波动更小。

6. **综合表现**：综合考虑训练和推理性能以及内存占用，两个框架各有优势：Jittor在训练速度和内存效率方面表现更好，而PyTorch在推理速度和检测精度上更有优势。
...
6. 进一步分析Jittor内存管理机制的优势，探索在大模型训练中的应用潜力。
